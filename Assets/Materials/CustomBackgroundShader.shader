    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                float inside0 = step(roundRect(i.uv, _RectPosition0.xy, _RectSize0.xy, _RectRadii0.x), 0.0);
                float inside1 = step(roundRect(i.uv, _RectPosition1.xy, _RectSize1.xy, _RectRadii1.x), 0.0);
                float inside2 = step(roundRect(i.uv, _RectPosition2.xy, _RectSize2.xy, _RectRadii2.x), 0.0);
                float inside3 = step(roundRect(i.uv, _RectPosition3.xy, _RectSize3.xy, _RectRadii3.x), 0.0);
                float inside4 = step(roundRect(i.uv, _RectPosition4.xy, _RectSize4.xy, _RectRadii4.x), 0.0);
                float inside0_1 = step(roundRect(i.uv, _RectPosition0_1.xy, _RectSize0_1.xy, _RectRadii0_1.x), 0.0);
                float inside1_2 = step(roundRect(i.uv, _RectPosition1_2.xy, _RectSize1_2.xy, _RectRadii1_2.x), 0.0);
                float inside2_3 = step(roundRect(i.uv, _RectPosition2_3.xy, _RectSize2_3.xy, _RectRadii2_3.x), 0.0);
                float inside3_4 = step(roundRect(i.uv, _RectPosition3_4.xy, _RectSize3_4.xy, _RectRadii3_4.x), 0.0);

                float insideAny = max(inside0, max(inside1, max(inside2, max(inside3, max(inside4, max(inside0_1, max(inside1_2, max(inside2_3, inside3_4))))))));
                return lerp(mainColor, secondaryColor, insideAny);
            }
            ENDCG
        }
    }
